...
public class SmelterRecipeManager extends AbstractManager implements IRecipeManager {
        ...
        switch (recipe.getInputItems().size()) {
            case 1:
                for (ItemStack firstInput : recipe.getInputItems().get(0).getMatchingStacks()) {
                    addRecipe(recipe.getEnergy(), recipe.getExperience(), recipe.getMinPower(), recipe.getMaxPower(), Collections.singletonList(firstInput), recipe.getInputFluids(), recipe.getOutputItems(), recipe.getOutputItemChances(), recipe.getOutputFluids());
                }
                return;
            case 2:
                for (ItemStack firstInput : recipe.getInputItems().get(0).getMatchingStacks()) {
                    for (ItemStack secondInput : recipe.getInputItems().get(1).getMatchingStacks()) {
                        addRecipe(recipe.getEnergy(), recipe.getExperience(), recipe.getMinPower(), recipe.getMaxPower(), asList(firstInput, secondInput), recipe.getInputFluids(), recipe.getOutputItems(), recipe.getOutputItemChances(), recipe.getOutputFluids());
                    }
                }
                return;
            case 3:
                for (ItemStack firstInput : recipe.getInputItems().get(0).getMatchingStacks()) {
                    for (ItemStack secondInput : recipe.getInputItems().get(1).getMatchingStacks()) {
                        for (ItemStack thirdInput : recipe.getInputItems().get(2).getMatchingStacks()) {
                            addRecipe(recipe.getEnergy(), recipe.getExperience(), recipe.getMinPower(), recipe.getMaxPower(), asList(firstInput, secondInput, thirdInput), recipe.getInputFluids(), recipe.getOutputItems(), recipe.getOutputItemChances(), recipe.getOutputFluids());
                        }
                    }
                }
                return;
    ...
    protected IMachineRecipe addRecipe(int energy, float experience, int minPower, int maxPower, List<ItemStack> inputItems, List<FluidStack> inputFluids, List<ItemStack> outputItems, List<Float> chance, List<FluidStack> outputFluids) {
        ...
        InternalSmelterRecipe recipe = new InternalSmelterRecipe(energy, experience, minPower, maxPower, inputItems, inputFluids, outputItems, chance, outputFluids);
    ...
    protected static class InternalSmelterRecipe extends CatalyzedMachineRecipe {
        ...
        public InternalSmelterRecipe(int energy, float experience, int minPower, int maxPower, @Nullable List<ItemStack> inputItems, @Nullable List<FluidStack> inputFluids, @Nullable List<ItemStack> outputItems, @Nullable List<Float> chance, @Nullable List<FluidStack> outputFluids) {

            super(3, energy, experience, minPower, maxPower, inputItems, inputFluids, outputItems, chance, outputFluids);
        }
